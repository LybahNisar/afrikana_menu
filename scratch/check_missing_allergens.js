...
console.log('Unique allergen items:', uniqueAllergens.size);
console.log('HTML menu items:', Object.keys(htmlMapping).length);

const missingItems = [];
Object.keys(htmlMapping).forEach(menuItem => {
  if (!uniqueAllergens.has(menuItem)) {
    missingItems.push(menuItem);
  }
});

console.log('\nHTML items missing from Allergen list:');
